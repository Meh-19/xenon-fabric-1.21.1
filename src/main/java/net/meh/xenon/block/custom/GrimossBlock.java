...
public class GrimossBlock extends Block {

    private static final VoxelShape SHAPE =
            Block.createCuboidShape(0, 0, 0, 16, 1, 16);

    public GrimossBlock(Settings settings) {
        super(settings);
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext context) {
        ...
        if (above.isSideSolidFullSquare(world, pos.up(), Direction.DOWN)
                || above.isOf(ModBlocks.GRIMOSS_HANG)
                || above.isOf(ModBlocks.GRIMOSS_HANG_BOTTOM)) {

            return ModBlocks.GRIMOSS_HANG.getDefaultState();
        }
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        return above.isSideSolidFullSquare(world, pos.up(), Direction.DOWN)
                || above.isOf(ModBlocks.GRIMOSS_HANG)
                || above.isOf(ModBlocks.GRIMOSS_HANG_BOTTOM)
                || below.isOf(ModBlocks.GRIMOSS_HANG)
                || below.isOf(ModBlocks.GRIMOSS_HANG_BOTTOM);
    }
